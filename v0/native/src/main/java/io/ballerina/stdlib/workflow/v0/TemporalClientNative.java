...
package io.ballerina.stdlib.workflow.v0;
...
/**
 * Native implementation for Temporal client initialization and lifecycle (V0).
 */
public class TemporalClientNative {
    ...
    public static Object initClient(BMap<BString, Object> config) {
        try {
            String serviceUrl = config.get(StringUtils.fromString("serviceUrl")).toString();
            String namespace = config.get(StringUtils.fromString("namespace")).toString();
            ...
            if (config.containsKey(StringUtils.fromString("identity"))) {
                String identity = config.get(StringUtils.fromString("identity")).toString();
    ...
    public static Object closeClient(Object clientHandle) {
        try {
            // WorkflowClient cleanup is handled automatically
            return null;
        } catch (Exception e) {
            return ErrorCreator.createError(
